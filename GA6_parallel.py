...
def parallel_distance2(i, bags, grupo, tipos):
   ...
   # _, score, _ = Cpx.biuld_classifier(X_bag, y_bag, X_bag, y_bag)
    _, score, pred = Cpx.biuld_classifier(X_bag, y_bag, X_bag, y_bag,X_vali,y_vali)
    #######################################################################
...
def the_function(population, gen, fitness):
    ...
    global geracao, off, dispersao, nr_generation, bags, local, arquivo_de_saida, accuracia_ant, s, c
    #print (c)
    #print (len(c))
   ...
   # exit(0)
    ...
    ###############################################333
    if repeticao == 1:
        with open(nome_base +str(gen)+ 'medidasevolucao20geracoes.csv', 'a', newline='') as csvfile:
             ...
             spamwriter.writerow('\n')
             if gen == nr_generation:
                 for i in tem2:
                     spamwriter.writerow(i)
        del tem, k, m1, m2, temp
    ###################################################3
    print("the_fuction")
    off = []
    s = []
...
arquivo_de_saida = "20distdiverlinear20"
